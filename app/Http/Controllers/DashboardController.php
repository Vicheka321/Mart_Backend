...
class DashboardController extends Controller
{
    ...
    public function admin()
    {
        ...
        /*
    |--------------------------------------------------------------------------
    | Recent Orders (dashboard table, with actions)
    |--------------------------------------------------------------------------
    | Respects the same date range filter as the rest of the dashboard.
    | FIFO "first pending" id is computed GLOBALLY (not date-limited),
    | matching the rule enforced on the main Orders page, so Accept/Reject
    | here stay consistent no matter which range is selected.
    */
        $recentOrdersQuery = OrderModel::with(['user', 'payment'])
