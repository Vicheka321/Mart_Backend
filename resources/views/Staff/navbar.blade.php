        ...
        <div class="flex items-center gap-5">
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-cloak x-show="open" @click.outside="open = false"
                    x-transition:enter="transition ease-out duration-200" x-transition:enter-start=" scale-95"
...
<script defer>
    // ==============================
    // One init function, called twice
    // ==============================
    function initNavbar() {
        ...
        window.addNotification = function (order) {
            ...
            const html = `
                <div onclick="goOrder(${order.id})"
                    ...
                    <div class="w-9 h-9 flex items-center justify-center rounded-full
                                bg-indigo-100 dark:bg-indigo-500/20
                                text-indigo-600 dark:text-indigo-400">🛒</div>
                    <div class="flex-1 text-sm">
                        <p class="text-gray-700 dark:text-gray-200 font-medium">New Order #${order.id}</p>
                        <p class="text-xs text-gray-400">$${order.total_amount}</p>
                    </div>
                    <span class="w-2 h-2 bg-blue-500 rounded-full mt-2"></span>
                </div>`;
            list.insertAdjacentHTML('afterbegin', html);
    ...
    // ==============================
    // Call on first load AND SPA nav
    // ==============================
