...
@section('content')
    ...
    {{--
            The controller now passes $permissionGroups already grouped by the
            `resource` column (Permission::where(...)->groupBy('resource')).
            We just re-shape it here into { "Resource Name" => [permission names...] }
            so the rest of this view (which was written against that shape)
            doesn't need to change.

            Within each resource group, a permission starting with "view_" is
            treated as the "main" permission (checking it selects all subs).
            If no "view_" permission exists in the group, the first one
            ...
            $permissionGroups = $permissionGroups
                ->map(function ($group) {
                    return $group
                        ->pluck('name')
                        ->sort(function ($a, $b) {
                            $aIsView = Str::startsWith($a, 'view_') ? 0 : 1;
                            $bIsView = Str::startsWith($b, 'view_') ? 0 : 1;

                            return $aIsView <=> $bIsView ?: strcmp($a, $b);
                        })
                        ->values()
                        ->toArray();
                })
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500">Set a name and choose permissions by resource.</p>
                            </div>
                        </div>
