...
@section('content')
    ...
    <style>
        ...
        /* Table wrapper */
        .table-card {
            animation: fadeSlideUp .5s .26s ease both;
        }
    ...
    <div class="space-y-4">
        ...
        <div
            ...
            <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-700
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-wrap">
                    ...
                    <div class="inline-flex items-center rounded-xl border border-gray-200 dark:border-gray-600
                        ...
                        <button type="button" id="gridViewBtn" onclick="setViewMode('grid')" title="Grid view"
                        ...
                        <button type="button" id="listViewBtn" onclick="setViewMode('list')" title="List view"
                            class="view-toggle-btn w-9 h-8 flex items-center justify-center rounded-lg">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                    ...
                    {{-- ADD --}}
                    <button type="button" onclick="openModal()" class="action-btn inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium rounded-xl
            ...
            <div id="gridViewWrap" class="p-4 sm:p-5">
                <div id="brandsTable" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">

                    @forelse($brands as $brand)
                        <div class="brand-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700
                                            rounded-2xl overflow-hidden flex flex-col"
                            data-name="{{ strtolower($brand->name) }}" data-country="{{ strtolower($brand->country ?? '') }}">
                            ...
                            @else
                                <div
                                    class="aspect-[4/3] bg-gradient-to-br from-gray-100 to-gray-200
                            ...
                            <div class="p-3 flex flex-col gap-2 flex-1">
                                ...
                                <div class="flex gap-1.5 mt-auto pt-1">
                                    <button type="button"
                                        onclick="editBrand({{ $brand->id }}, '{{ addslashes($brand->name) }}', '{{ addslashes($brand->country ?? '') }}', '{{ $brand->image ? asset($brand->image) : '' }}')"
                                    ...
                                    <form action="{{ route('brands.destroy', $brand->id) }}" method="POST"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit" class="action-btn w-full inline-flex items-center justify-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg
                                                        border border-gray-200
                                                        bg-white
                                                        text-gray-600
                                                        hover:bg-gray-50
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30">
                            <th
                                class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                                Brand</th>
                            <th
                                class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                                Country</th>
                            <th
                                ...
                                Logo</th>
                            <th
                                class="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($brands as $brand)
                            <tr class="list-row" data-name="{{ strtolower($brand->name) }}"
                                data-country="{{ strtolower($brand->country ?? '') }}">

                                <td class="px-5 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0
                                                            {{ $brand->image ? 'bg-gray-50 dark:bg-gray-700' : 'bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600' }}
                                                            flex items-center justify-center">
                                            @if($brand->image)
                                                <img src="{{ asset($brand->image) }}" class="w-full h-full object-cover">
                                            @else
            ...
            <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500">
                    @if($brands->total())
                        Showing
                        <span
                            class="font-semibold text-gray-700 dark:text-gray-200">{{ $brands->firstItem() }}–{{ $brands->lastItem() }}</span>
                        of
                        <span
                            class="font-semibold text-gray-700 dark:text-gray-200">{{ number_format($brands->total()) }}</span>
                        results
                    @else
                        No brands found
                    @endif
                ...
                @if($brands->hasPages())
                    ...
                    <nav class="flex items-center gap-1">
                        ...
                        @else
                            <a href="{{ $brands->previousPageUrl() }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 dark:text-gray-400
                        ...
                        @if($start > 1)
                            <a href="{{ $brands->url(1) }}" class="min-w-[32px] h-8 px-2 inline-flex items-center justify-center rounded-lg
                        ...
                        {{-- Page Numbers --}}
                        @foreach($brands->getUrlRange($start, $end) as $page => $url)
                            @if($page == $current)
                                <span
                        ...
                        @if($end < $last)
                            @if($end < $last - 1)
                                <span
                                    class="w-8 h-8 flex items-center justify-center text-sm text-gray-300 dark:text-gray-600 select-none">…</span>
                            @endif
                            <a href="{{ $brands->url($last) }}" class="min-w-[32px] h-8 px-2 inline-flex items-center justify-center rounded-lg
                                                  text-sm font-medium text-gray-500 dark:text-gray-400
                                                  hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white
                                                  transition-colors duration-150">
                                {{ $last }}
                        ...
                        @if($brands->hasMorePages())
                            <a href="{{ $brands->nextPageUrl() }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 dark:text-gray-400
                                                hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white
                                                transition-colors duration-150">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div id="exportModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50 p-4">
        <div class="modal-inner bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700
            ...
            <div class="p-6 space-y-3">
                ...
                <a href="{{ route('brands.export.csv') }}" class="group flex items-center justify-between px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600
                ...
                <a href="{{ route('brands.export.pdf') }}" class="group flex items-center justify-between px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600
            ...
            <form id="brandForm" action="{{ route('brands.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="name" id="brandName" placeholder="e.g. Nike" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-600
                                       bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-white placeholder-gray-400
                                       focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                        <p id="brandNameError" class="hidden mt-1 text-xs text-red-500"></p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Country</label>
                        <input type="text" name="country" id="brandCountry" placeholder="e.g. USA" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-600
                                       bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-white placeholder-gray-400
                                       focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div id="uploadBox" onclick="document.getElementById('imageInput').click()" class="relative w-full h-44 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-600
                        ...
                        <div id="uploadPlaceholder"
                            ...
                            <div
                                class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-600 flex items-center justify-center">
                        ...
                        <div id="editOverlay"
                            ...
                            <span
                                class="bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-xs font-medium px-4 py-1.5 rounded-full shadow">
                                Change Photo
                            </span>
                        </div>

                        <button type="button" id="removeImageBtn" onclick="removeImage(event)" class="hidden absolute top-2 right-2 w-7 h-7 bg-gray-900/70 hover:bg-red-500 text-white
    ...
    @push('scripts')
        <script>
            ...
            function setViewMode(mode) {
                ...
                try { localStorage.setItem('brandsViewMode', mode); } catch (e) { }
            }
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                let savedMode = 'grid';
                try { savedMode = localStorage.getItem('brandsViewMode') || 'grid'; } catch (e) { }
            ...
            function clearFieldErrors() {
                ...
                const error = document.getElementById('brandNameError');
            ...
            function filterBrands() {
                ...
                const cards = document.querySelectorAll('.brand-card, .list-row');
