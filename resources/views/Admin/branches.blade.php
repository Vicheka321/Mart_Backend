...
@section('content')
    ...
    <style>
        /* All original styles kept exactly as-is (no Alpine involved here) */
            ...
            .bx-header { animation: fadeSlideUp .45s ease both; }
            ...
            .bx-row { animation: rowSlideIn .3s ease both; }
            .bx-modal-overlay.flex { animation: overlayIn .2s ease; }
            ...
            .bx-hover-lift { transition: box-shadow .3s ease, transform .3s ease; }
            .bx-hover-lift:hover { box-shadow: 0 8px 24px rgba(0, 0, 0, .08); transform: translateY(-2px); }
            ...
            #branchMap, #viewMap { border-radius: 16px; overflow: hidden; }
            ...
            #productSearchB:focus { box-shadow: 0 0 0 3px rgba(79, 70, 229, .15); }
            ...
            #filterDropdownBtn .bx-chevron { transition: transform .15s ease; }
            #filterDropdownBtn.open .bx-chevron { transform: rotate(180deg); }
        </style>
        ...
        <div id="branchesPage" class="space-y-4">
            ...
            <div class="table-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
                ...
                <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-700
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-wrap w-full lg:w-auto">
                        ...
                        <div class="relative w-full sm:w-auto">
                            ...
                            <input type="text" id="productSearchB" placeholder="Search branches…"
                                autocomplete="off" class="w-full sm:w-56 md:w-64 pl-10 pr-4 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-600
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <div class="relative" id="filterDropdownWrap">
                                <button type="button" id="filterDropdownBtn"
                                    ...
                                    <svg class="w-3.5 h-3.5 text-gray-400 bx-chevron"
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr
                                ...
                                <th class="px-3 sm:px-5 py-3">Phone</th>
                                <th class="px-3 sm:px-5 py-3">Email</th>
                                <th class="px-3 sm:px-5 py-3">Address</th>
                                <th class="px-3 sm:px-5 py-3">Coordinates</th>
                                <th class="px-3 sm:px-5 py-3 text-center">Main</th>
                                <th class="px-3 sm:px-5 py-3 text-center">Status</th>
                                <th class="px-3 sm:px-5 py-3">Updated</th>
                ...
                {{-- ══════ Mobile cards ══════ --}}
                <div class="md:hidden p-3 sm:p-4 space-y-3" id="branchMobileList">
                    {{-- Populated entirely by JS: skeleton cards, data cards, or empty message --}}
                </div>

                {{-- Empty state (first-run, no branches at all) --}}
                <div id="emptyState" class="bx-hidden flex-col items-center justify-center text-center py-16 px-6 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <form id="branchForm">
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700/40 rounded-2xl p-3 sm:p-4 space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Branch
                                            Name</label>
                                        <input type="text" id="formName" placeholder="e.g. Phnom Penh HQ" class="w-full px-3 py-2 text-sm rounded-xl border bg-white dark:bg-gray-700 text-gray-800 dark:text-white
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="branchAddressInput"
                                            placeholder="Search an address in Cambodia…" class="w-full px-3 py-2 text-sm rounded-xl border bg-white dark:bg-gray-700 text-gray-800 dark:text-white
                                                          focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all border-gray-200 dark:border-gray-600">
                        ...
                        <div
                            ...
                            <button type="submit" id="branchSubmitBtn" class="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium text-white
                                ...
                                <svg id="branchSubmitSpinner" class="w-4 h-4 animate-spin bx-hidden" viewBox="0 0 24 24" fill="none">
                    ...
                    <div>
                        <div class="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700">
                            <div class="flex items-center gap-3 min-w-0">
                                <div id="viewAvatar" class="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white text-sm font-bold shrink-0"></div>
                                <div class="min-w-0">
                                    <h3 class="text-base font-semibold text-gray-900 dark:text-white truncate" id="viewName"></h3>
                        ...
                        <div class="p-4 sm:p-6 space-y-3">
                            <div class="grid grid-cols-2 gap-3">
                                <div class="bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
                                    <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Phone</p>
                                    <p class="text-xs font-medium text-gray-900 dark:text-white" id="viewPhone"></p>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
                                    <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Email</p>
                                    <p class="text-xs font-medium text-gray-900 dark:text-white truncate" id="viewEmail"></p>
                                </div>
                            </div>

                            <div class="bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
                                <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Address</p>
        ...
        @push('scripts')
                ...
                (function () {
                    ...
                    const BranchPage = {
                        ...
                        init() {
                            ...
                            this.bindEvents();
                            this.renderTable(); // shows skeleton rows immediately
                        ...
                        bindEvents() {
                            ...
                            // Table body: event delegation for view / edit / delete
                            document.getElementById('branchTableBody').addEventListener('click', (e) => this.handleTableClick(e));
                            document.getElementById('branchMobileList').addEventListener('click', (e) => this.handleTableClick(e));
                        },
                        ...
                        renderTable() {
                            ...
                            if (this.tableLoading) {
                                ...
                                mobileList.innerHTML = this.skeletonCardsHtml(3);
                                hide(emptyState);
                                return;
                            }
                            ...
                            if (list.length === 0) {
                                ...
                                mobileList.innerHTML = `<div class="py-12 text-center text-sm text-gray-400 dark:text-gray-500">No branches found.</div>`;
                            } else {
                                tbody.innerHTML = list.map((b, i) => this.rowHtml(b, i)).join('');
                                mobileList.innerHTML = list.map(b => this.mobileCardHtml(b)).join('');
                            }
                            ...
                            for (let i = 0; i < n; i++) {
                                rows += `
                                    <tr>
                                        ...
                                        <td class="px-3 sm:px-5 py-4"><div class="bx-skel h-3 w-24"></div></td>
                                        <td class="px-3 sm:px-5 py-4"><div class="bx-skel h-3 w-28"></div></td>
                                        <td class="px-3 sm:px-5 py-4"><div class="bx-skel h-3 w-40"></div></td>
                                        <td class="px-3 sm:px-5 py-4"><div class="bx-skel h-3 w-24"></div></td>
                                        ...
                                        <td class="px-3 sm:px-5 py-4"><div class="bx-skel h-3 w-16"></div></td>
                        ...
                        skeletonCardsHtml(n) {
                            ...
                            for (let i = 0; i < n; i++) {
                                cards += `
                                    <div class="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 space-y-2.5">
                                ...
                                <tr class="bx-row hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors duration-150">
                                    ...
                                    <td class="px-3 sm:px-5 py-3.5">
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <span class="text-sm font-semibold text-gray-900 dark:text-white">${escapeHtml(branch.name)}</span>
                                        </div>
                                    </td>
                                    <td class="px-3 sm:px-5 py-3.5 text-gray-600 dark:text-gray-300">${escapeHtml(branch.phone || '—')}</td>
                                    <td class="px-3 sm:px-5 py-3.5 text-gray-600 dark:text-gray-300">${escapeHtml(branch.email || '—')}</td>
                                    <td class="px-3 sm:px-5 py-3.5 text-gray-600 dark:text-gray-300 max-w-[220px] truncate" title="${escapeHtml(branch.address || '')}">${escapeHtml(branch.address || '—')}</td>
                                    <td class="px-3 sm:px-5 py-3.5">
                                    ...
                                    <td class="px-3 sm:px-5 py-3.5 text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap">${this.formatDate(branch.updated_at)}</td>
                                    <td class="px-3 sm:px-5 py-3.5">
                        ...
                        mobileCardHtml(branch) {
                            ...
                            const editBtn = CAN_EDIT ? `
                                <button type="button" data-action="edit" data-id="${branch.id}"
                                    class="action-btn w-8 h-8 rounded-lg flex items-center justify-center border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                                    </svg>
                            ...
                            const deleteBtn = CAN_DELETE ? `
                                <button type="button" data-action="delete" data-id="${branch.id}" ${branch.is_main ? 'disabled' : ''}
                                    class="action-btn w-8 h-8 rounded-lg flex items-center justify-center border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 disabled:opacity-40">
                                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M7 4a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v2h4a1 1 0 1 1 0 2h-1.069l-.867 12.142A2 2 0 0 1 17.069 22H6.93a2 2 0 0 1-1.995-1.858L4.07 8H3a1 1 0 0 1 0-2h4V4zm2 2h6V4H9v2zM6.074 8l.857 12H17.07l.857-12H6.074zM10 10a1 1 0 0 1 1 1v6a1 1 0 1 1-2 0v-6a1 1 0 0 1 1-1zm4 0a1 1 0 0 1 1 1v6a1 1 0 1 1-2 0v-6a1 1 0 0 1 1-1z" />
                                    </svg>
                                </button>` : '';
                            ...
                            return `
                                <div class="bx-row rounded-2xl border border-gray-200 dark:border-gray-700 p-4">
                                    <div class="flex items-start justify-between gap-2 mb-2.5">
                                        <div class="flex items-center gap-2.5 min-w-0">
                                            <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white text-xs font-bold shrink-0">${escapeHtml((branch.name || '?').charAt(0).toUpperCase())}</div>
                                            <div class="min-w-0">
                                                <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">${escapeHtml(branch.name)}</p>
                                                <p class="text-[11px] text-gray-400 dark:text-gray-500 truncate">${escapeHtml(branch.address || '—')}</p>
                                            </div>
                                        </div>
                                        <span class="bx-badge shrink-0 ${statusClasses}">${branch.status === 'active' ? 'Active' : 'Inactive'}</span>
                                    </div>

                                    <div class="grid grid-cols-2 gap-2 text-[11px] text-gray-500 dark:text-gray-400 mb-3">
                                        <div><span class="text-gray-400 dark:text-gray-500">Phone:</span> ${escapeHtml(branch.phone || '—')}</div>
                                        <div><span class="text-gray-400 dark:text-gray-500">Email:</span> ${escapeHtml(branch.email || '—')}</div>
                                        <div class="font-mono"><span class="text-gray-400 dark:text-gray-500 font-sans">Lat:</span> ${branch.lat ? Number(branch.lat).toFixed(6) : '—'}</div>
                                        <div class="font-mono"><span class="text-gray-400 dark:text-gray-500 font-sans">Lng:</span> ${branch.lng ? Number(branch.lng).toFixed(6) : '—'}</div>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            <button type="button" data-action="view" data-id="${branch.id}"
                                                class="action-btn w-8 h-8 rounded-lg flex items-center justify-center border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400">
                                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5s8.268 2.943 9.542 7c-1.274 4.057-5.065 7-9.542 7S3.732 16.057 2.458 12Z" />
                                                    <circle cx="12" cy="12" r="3" />
                                                </svg>
            ...
            <script src="https://maps.googleapis.com/maps/api/js?key={{ config('services.google.api_key') }}&libraries=places&callback=initGoogleMaps" async defer></script>
        @endpush
