...
@section('content')
    <style>
        ...
        @keyframes slideInRight {
            ...
            .table-card { animation: fadeSlideUp .5s .35s ease both; }

            #productsTableBody tr { animation: rowSlideIn .35s ease both; }
            #productsTableBody tr:nth-child(1)  { animation-delay: .38s; }
            #productsTableBody tr:nth-child(2)  { animation-delay: .43s; }
            #productsTableBody tr:nth-child(3)  { animation-delay: .48s; }
            #productsTableBody tr:nth-child(4)  { animation-delay: .53s; }
            #productsTableBody tr:nth-child(5)  { animation-delay: .58s; }
            #productsTableBody tr:nth-child(6)  { animation-delay: .63s; }
            #productsTableBody tr:nth-child(7)  { animation-delay: .68s; }
            #productsTableBody tr:nth-child(8)  { animation-delay: .73s; }
            #productsTableBody tr:nth-child(9)  { animation-delay: .78s; }
            #productsTableBody tr:nth-child(10) { animation-delay: .83s; }
            ...
            .pr-btn { transition: transform .15s ease, box-shadow .15s ease, background-color .15s ease; }
            .pr-btn:active { transform: translateY(1px); }

            .pr-dropdown.open,
            #prFilterOverlay.flex { animation: overlayIn .18s ease; }

            .pr-drawer { animation: slideInRight .28s cubic-bezier(.32,.72,0,1) both; }
            ...
            .dark .pr-input { border-color: rgb(55 65 81); background: rgb(31 41 55); color: rgb(243 244 246); }
            .pr-input:focus { box-shadow: 0 0 0 3px rgba(99,102,241,.25); border-color: #6366f1; }

            .pr-scrollbar::-webkit-scrollbar { width: 6px; height: 6px; }
            .pr-scrollbar::-webkit-scrollbar-thumb { background: rgba(120,120,140,.25); border-radius: 999px; }
        </style>
        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3"
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Products Report</h1>
                ...
                <div class="flex items-center gap-2 relative">
                    ...
                    <button type="button" onclick="openPrFilterDrawer()"
                        class="pr-btn inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500
                               text-white text-xs font-semibold shadow-lg shadow-indigo-500/20">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M3 4h18l-7 8v6l-4 2v-8L3 4z"/>
                        </svg>
                        Filter
                        @php $prActiveCount = count(array_filter(request()->only(['category', 'brand', 'stock_status', 'sort', 'keyword']))); @endphp
                        @if($prActiveCount)
                            <span class="ml-0.5 inline-flex items-center justify-center w-4 h-4 rounded-full bg-white text-indigo-600 text-[10px] font-bold">{{ $prActiveCount }}</span>
                        @endif
                    </button>

                    {{-- ---- Export dropdown ---- --}}
                    <div class="relative">
                        <button type="button" onclick="toggleDropdown('prExportDropdown')"
                            class="pr-btn inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700
                        ...
                        <div id="prExportDropdown" class="pr-dropdown hidden absolute right-0 mt-2 w-48 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-2xl shadow-black/10 dark:shadow-black/40 z-40 overflow-hidden p-1.5">
            ...
            @if($prActiveCount)
                @php
                    $prFilterLabels = [
                        ...
                        'stock_status' => ['instock' => 'In Stock', 'lowstock' => 'Low Stock', 'outstock' => 'Out of Stock'][request('stock_status')] ?? request('stock_status'),
                        'sort' => ['price_high' => 'Price: High → Low', 'price_low' => 'Price: Low → High', 'stock' => 'Stock: High → Low'][request('sort')] ?? request('sort'),
                        'keyword' => request('keyword'),
                    ];
                @endphp
                <div class="flex flex-wrap items-center gap-2">
                    @foreach($prFilterLabels as $k => $label)
                        @if(request($k) && $label)
                            <span class="inline-flex items-center gap-1.5 pl-3 pr-2 py-1 rounded-full bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-[11px] text-gray-600 dark:text-gray-300">
                                ...
                                <a href="{{ route('reports.products', request()->except($k, 'page')) }}" class="text-gray-400 hover:text-rose-500">
                                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M18 6 6 18M6 6l12 12"/></svg>
                                </a>
                            </span>
                        @endif
                    @endforeach
                    <a href="{{ route('reports.products') }}" class="text-[11px] text-gray-400 hover:text-rose-500 font-medium">Clear all</a>
                </div>
            @endif
                ...
                <div class="chart-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
                    <div class="px-4 sm:px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2">
                        <div class="w-6 h-6 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center">
                            <svg class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7M4 7c0-2 1-3 3-3h10c2 0 3 1 3 3M4 7h16M10 11v4M14 11v4"/>
                            </svg>
                        </div>
                            ...
                            @forelse($products as $product)
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="w-28 mx-auto">
                                            <div class="flex justify-between mb-1">
                                                <span class="text-xs font-semibold text-gray-900 dark:text-white">{{ number_format($qty) }}</span>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-semibold {{ $stockBg }} {{ $stockText }}">
                                ...
                                <tr>
                                    <td colspan="8" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-sm text-gray-400 dark:text-gray-500">No products found. Try adjusting your filters.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        {{-- ==================== FILTER DRAWER ==================== --}}
        <div id="prFilterOverlay" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden z-50" onclick="closePrFilterDrawer()"></div>
        <div id="prFilterDrawer" class="pr-drawer fixed top-0 right-0 h-full w-full sm:w-[420px] bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 z-50 hidden flex-col shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                ...
                <button type="button" onclick="closePrFilterDrawer()" class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-300">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
                </button>
            </div>

            <form method="GET" action="{{ route('reports.products') }}" class="flex-1 overflow-y-auto pr-scrollbar p-5 space-y-4">
                ...
                <div>
                    ...
                    <select name="category" class="pr-input">
                ...
                <div>
                    ...
                    <select name="brand" class="pr-input">
                ...
                <div>
                    ...
                    <select name="stock_status" class="pr-input">
                ...
                <div>
                    ...
                    <select name="sort" class="pr-input">
                ...
                <div class="pt-2 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <input type="text" name="keyword" value="{{ request('keyword') }}"
                           placeholder="Product name…" class="pr-input">
                </div>

                <div class="sticky bottom-0 pt-4 pb-1 bg-white dark:bg-gray-800 flex items-center gap-2">
                    <a href="{{ route('reports.products') }}" class="flex-1 text-center py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-xs font-semibold hover:bg-gray-50 dark:hover:bg-gray-700">Reset all</a>
        ...
        @push('scripts')
            ...
            <script defer>
                ...
                function toggleDropdown(id) {
                    document.querySelectorAll('.pr-dropdown').forEach(el => { if (el.id !== id) el.classList.add('hidden'); });
                    document.getElementById(id).classList.toggle('hidden');
                }
                document.addEventListener('click', function (e) {
                    document.querySelectorAll('.pr-dropdown').forEach(el => {
                ...
                // ── Filter drawer ─────────────────────────────────────────────
                const prFilterDrawer  = document.getElementById('prFilterDrawer');
                const prFilterOverlay = document.getElementById('prFilterOverlay');
                function openPrFilterDrawer() {
                    prFilterOverlay.classList.remove('hidden');
                    prFilterDrawer.classList.remove('hidden');
                    prFilterDrawer.classList.add('flex');
                    document.body.classList.add('overflow-hidden');
                }
                function closePrFilterDrawer() {
                    prFilterOverlay.classList.add('hidden');
                    prFilterDrawer.classList.add('hidden');
                    prFilterDrawer.classList.remove('flex');
                ...
                document.addEventListener('keydown', e => {
                    if (e.key === 'Escape') { closeProductModal(); closePrFilterDrawer(); }
                });
